...
stache.startsub = function() {
    stache.sub = true;
};

stache.endsub = function() {
    while(true) {
        if(!stache.sub) {
            break;
        }
...
stache.subready = function() {
    stache.log('sready'); //testing
    stache.sub = false;
};

stache.argvloop = function(val, i, argv) { //argument loop
    fs.appendFileSync('dbg.js', '/*TOP*/\n'+JSON.stringify(stache), 'utf-8'); //testing
    ...
    if(i==argv.length-1) { //if we are at the end of the loop
        if(stache.js !== false) { //if the source name is set
            ...
            if(stache.usetags) { //if we are importing tags
                ...
                var _buff = new Buffer(stache.tagstr + fs.readFileSync(stache.js, 'utf-8'), 'utf-8'); //combine the tags + stache.js data and store in buffer
                fs.writeSync(_fd, _buff, 0, _buff.length); //write the entire buffer to the temp file
                fs.closeSync(_fd); //close the temp file
                
                stache.js = _stachejs.path; //set the stache.js path to the temp file path
            }
            
            stache.log('stacheeeeeeeeeeeeeee'); //testing
            
            (mu.compileAndRender(stache.js, stache.view)).addListener('data', function (data) { //compile and render using mustache
                stache.log('dataa'); //testing
                ...
                fs.appendFileSync('dbg.js', '/*END IS*/\n'+JSON.stringify(stache), 'utf-8'); //testing
                stache.subready();
            }).addListener('error', function(err) {
                stache.error(err);
            }).addListener('close', function() {
                stache.log('stream closed');
            });
            stache.subready();
    ...
    fs.appendFileSync('dbg.js', '/*BOTTOM*/\n'+JSON.stringify(stache), 'utf-8'); //testing
};
