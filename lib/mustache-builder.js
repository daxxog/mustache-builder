    ...
    if(i==argv.length-1) { //if we are at the end of the loop
        if(stache.js !== false) { //if the source name is set
            if(stache.usetags) { //if we are importing tags
                ...
                _fs.writeSync(stache.tagstr + fs.readFileSync(stache.js, 'utf-8')); //write the tags + stache.js data to the temp file
                _fs.closeSync(); //close the temp file
